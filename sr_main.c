...
#define DEFAULT_ICMP_TIMEOUT 44
#define DEFAULT_TCP_E_TIMEOUT 44
#define DEFAULT_TCP_T_TIMEOUT 44
...
int main(int argc, char **argv)
{
    ...
    int tcp_e_timeout = DEFAULT_TCP_E_TIMEOUT;
    int tcp_t_timeout = DEFAULT_TCP_T_TIMEOUT;
    int nat = 0;
    ...
    while ((c = getopt(argc, argv, "hs:v:p:u:t:r:l:T:")) != EOF)
    {
        switch (c)
        {
            ...
            case 'I':
                ...
                break;
            case 'E':
                tcp_e_timeout = atoi((char*) optarg);
                break;
            case 'R':
                tcp_t_timeout = atoi((char*) optarg);
                break;
        } /* switch */
    } /* -- while -- */
    ...
    if(nat == 1){
        sr_nat_init(&(sr.nat), icmp_timeout, tcp_t_timeout, tcp_e_timeout);
        sr.nat_enabled = 1;
    }else{
        sr.nat_enabled = 0;
    }
